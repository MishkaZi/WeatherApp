  ...
  const fetchCityData = useCallback(async () => {
    ...
    try {
      const currentData = await axios.get(
        `https://dataservice.accuweather.com/currentconditions/v1/${detailedForecast.cityId}?apikey=${process.env.REACT_APP_ACCUWEATHER_API}`
      );
      ...
      const { data: forecastsData } = await axios.get(
        `https://dataservice.accuweather.com/forecasts/v1/daily/5day/${detailedForecast.cityId}?apikey=${process.env.REACT_APP_ACCUWEATHER_API}`
      );
